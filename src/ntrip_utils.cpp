...
std::string NTRIPUtils::base64_encode(const std::string &in) {
    std::string out;
    int val = 0, valb = -6;
...
std::string NTRIPUtils::base64_encode(const uint8_t* buf, size_t size)
{
    std::string out;
    int val = 0, valb = -6;
...
// bool NTRIPUtils::connect_and_stream() {
...
std::string NTRIPUtils::build_request_header() {
...
std::string NTRIPUtils::generate_gga_sentence() {
...
std::string NTRIPUtils::decimal_to_nmea(double deg, bool is_lat) {
...
// Overload for raw byte buffer + size
std::string NTRIPUtils::base64_encode(const char* buf, size_t size)
{
    return base64_encode(std::string(buf, size));
}

// void NTRIPUtils::parse_rtcm_messages(const char* data, size_t len) {
//     size_t i = 0;
//     while (i < len) {
